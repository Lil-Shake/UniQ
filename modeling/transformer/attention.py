...
def multi_head_attention_forward(query: Tensor,
    ...
    # if key.isnan().int().sum() > 0:
    #     for idx, x in enumerate(key):
    #         if x.isnan().int().sum() > 0:
    #             for idxy, y in enumerate(x):
    #                 if(y.isnan().int().sum()>0):
    #                     print(f'key:{key[idx][idxy]}')
    #                     print(f'value:{value[idx][idxy]}')
    # assert key.isnan().int().sum()== 0
    ...
    attn_output_weights = torch.bmm(q_scaled, k.transpose(1, 2))
    assert list(attn_output_weights.size()) == [bsz * num_heads, tgt_len, src_len]
    # if attn_output_weights.isnan().int().sum() > 0:
    #     for idx, x in enumerate(attn_output_weights):
    #         if x.isnan().int().sum() > 0:
    #             for idxy, y in enumerate(x):
    #                 if(y.isnan().int().sum()>0):
    #                     print(f'attn:{attn_output_weights[idx][idxy]}')
    #                     print(f'q:{q_scaled[idx][idxy]}')
    #                     print(f'k:{k.transpose(1, 2)[idx][idxy]}')
    # assert attn_output_weights.isnan().int().sum()== 0
    ...
    tmp_atten = attn_output_weights
    tmp = tmp_atten.isinf().sum(-1).unsqueeze(-1).repeat(1,1,tmp_atten.shape[-1])
    attn_output_weights = torch.where(tmp==tmp_atten.shape[-1], torch.full_like(tmp_atten, 1),tmp_atten)
    # if attn_output_weights.isnan().int().sum() > 0:
    #     for idx, x in enumerate(attn_output_weights):
    #         if x.isnan().int().sum() > 0:
    #             for idxy, y in enumerate(x):
    ...
    #             for idxy, y in enumerate(x):
    #                 if(y.isnan().int().sum()>0):
    #                     print(f'{idx}-{idxy} y: {y}')
    #                     print(f'{idx}-{idxy} weight: {attn_output_weights[idx][idxy]}')
    #                     for idxz, z in enumerate(y):
    #                         if(z.isnan().int().sum()>0):
    #                             print(f'{z}', end=" ")
    #                     print('')
    #                     for idxz, z in enumerate(y):
    #                         if(z.isnan().int().sum()>0):
    #                             print(f'{attn_output_weights[idx][idxy][idxz]}', end=" ")
    #                     print('')
    # attn_output_weights = tmp
    # if attn_output_weights.isnan().int().sum() > 0:
    ...
    # if attn_output.isnan().int().sum() > 0:
    #     print(attn_output)
    # assert attn_output.isnan().int().sum()== 0
