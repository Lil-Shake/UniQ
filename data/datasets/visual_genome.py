...
class VisualGenomeTrainData:
    ...
    def __init__(self, cfg, split='train'):
        ...
        self.trilevel_resample = cfg.DATASETS.VISUAL_GENOME.TRI_LEVEL if split == 'train' else False
        ...
        if self.trilevel_resample is True:
            freq = statistics['fg_rel_count'] / statistics['fg_rel_count'].sum() 
            freq = freq.numpy()
            sop_freq =  statistics['fg_matrix'][:-1,:-1,:-1] /  statistics['fg_matrix'][-1, -1, :-1].unsqueeze(0).unsqueeze(0)
            sop_freq = sop_freq.numpy()

            oversample_param = cfg.DATASETS.VISUAL_GENOME.OVERSAMPLE_PARAM # 0.07
            undersample_param = cfg.DATASETS.VISUAL_GENOME.UNDERSAMPLE_PARAM # 0.7
            oversampling_ratio = np.maximum(np.sqrt((oversample_param / (freq + 1e-5))), np.ones_like(freq))[:-1]
            sop_oversampling_ratio = np.maximum(np.sqrt((oversample_param / (sop_freq + 1e-5))), np.ones_like(sop_freq)) # [150, 150, 3]
            sampled_dataset_dicts = []
            sampled_num = []
            ...
            for record in self.dataset_dicts:
                ...
                if len(relations) > 0:
                    ...
                    repeat_num = int(np.ceil(np.max(oversampling_ratio[unique_relations]))) # select max oversampling in all predicates of the image
                    for rep_idx in range(repeat_num): # oversample image repeat_num times
                        sampled_num.append(repeat_num)
                        sampled_dataset_dicts.append(record)
                else:
                    sampled_dataset_dicts.append(record)
            ...
            self.dataset_dicts = sampled_dataset_dicts
            self.dataloader = TrilvlSampler(self.dataset_dicts, sampled_num, oversampling_ratio, sop_oversampling_ratio, undersample_param)
            DatasetCatalog.remove('VG_{}'.format(self.split))
            self.register_dataset(dataloader=True)
            MetadataCatalog.get('VG_{}'.format(self.split)).set(statistics=statistics) 
            print (self.idx_to_predicates, statistics['fg_rel_count'].numpy().tolist())
    ...
    def get_statistics(self, eps=1e-3, bbox_overlap=True):
        ...
        fg_matrix = np.zeros((num_object_classes, num_object_classes, num_relation_classes), dtype=np.int64)
        bg_matrix = np.zeros((num_object_classes-1, num_object_classes-1), dtype=np.int64)
        fg_rel_count = np.zeros((num_relation_classes), dtype=np.int64)
        for idx, data in enumerate(self.dataset_dicts):
            gt_relations = data['relations']
            gt_classes = np.array([x['category_id'] for x in data['annotations']])
            ...
            for (o1, o2) in gt_classes[np.array(box_filter(gt_boxes, must_overlap=bbox_overlap), dtype=int)]:
                bg_matrix[o1, o2] += 1
        if trilvl_ds is True:
            fg_matrix[:-1, :-1, -1] = bg_matrix
            fg_matrix[-1, -1, :-1] = fg_rel_count[:-1]
            fg_matrix[-1, -1, -1] = fg_rel_count[:-1].sum(0)
            result = {
                'fg_matrix': torch.from_numpy(fg_matrix),
                'fg_rel_count': torch.from_numpy(fg_rel_count).float(),
                'obj_classes': self.idx_to_classes + ['__background__'],
...
class TrilvlSampler(Dataset):
    """
    Wrap a list to a torch Dataset. It produces elements of the list as data.
    """

    def __init__(self, lst, sampled_num, oversampled_ratio, sop_oversampled_ratio, undersample_param):
        self._lst = lst
        self.sampled_num = sampled_num
        # self.sop_max = sop_max_list
        self.oversampled_ratio = oversampled_ratio
        self.sop_oversampled_ratio = sop_oversampled_ratio
        self.undersample_param = undersample_param
        # self.unique_relation_ratios = unique_relation_ratios
        # self.unique_sop_ratios = unique_sop_ratios
        # # self.unique_relations = unique_relations
        # self.unique_sop = unique_sop
    ...
    def __getitem__(self, idx):
        ...
        new_record = copy.deepcopy(record)
        if len(relations) > 0:
            unique_sop = np.unique(relations, axis=0)
            unique_sop_dict = {str(spo):idx for idx, spo in enumerate(unique_sop)}
            rc = self.oversampled_ratio[unique_sop[:,2]]
            ri = self.sampled_num[idx]
            rp = int(np.ceil(np.max(self.sop_oversampled_ratio[unique_sop])))
            scale = rp / ri
            rso = np.zeros_like(rc)
            for idx, sop in enumerate(unique_sop):
                rso[idx] = self.sop_oversampled_ratio[sop[0]][sop[1]][sop[2]] / scale
            dropout = np.clip(np.sqrt(((ri - rso)/ri) * ((ri - rc)/ri)) * self.undersample_param, 0.0, 1.0)
            random_arr = np.random.uniform(size=len(relations))
            index_arr = np.array([unique_sop_dict[str(sop)] for sop in relations])
            rel_dropout = dropout[index_arr]
            to_keep = rel_dropout < random_arr
            dropped_relations = relations[to_keep]
            new_record['relations'] = dropped_relations
