...
from .utils import intersect_2d, argsort_desc, intersect_2d_topk
...
class SceneGraphEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "instances" in output:
                ...
                prediction['pred_class_prob'] = output["pred_class_prob"].to(self._cpu_device)
    ...
    def _evaluate_scenegraphs(self, ground_truths, predictions):
        ...
        for k, v in self._evaluators['SGRecall'].result_dict[self._mode + '_recall'].items():
            print(f'Recall cal -> {k}: {np.mean(v)}')
            ret['SGRecall@{}'.format(k)] = float(np.mean(v))
    ...
    def evaluate_relation_of_one_image(self, groundtruth, prediction, global_container, i):
        ...
        local_container['rel_class_prob'] = prediction['pred_class_prob'].detach().cpu().numpy()
        # about objects
...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        # TODO
        pred_rel_labels = local_container['rel_labels']
        
        pred_scores = local_container['rel_scores']
        ...
        # pred_inds = rel_scores[:,:-1].argmax(1)
        pred_rels = np.column_stack((pred_rel_inds, pred_rel_labels)) #Backround index at the end
        # pred_scores = rel_scores[:,:-1].max(1) #Backround index at the end
        
        # sorted_indices = np.argsort(-rel_scores[:,:-1])[:, :3]
        # pred_rels = np.column_stack((pred_rel_inds, sorted_indices))
        
        # print(f'sorted_indices:{sorted_indices}')
        # print(f'sorted_indices shape:{sorted_indices.shape}')
        
        # print(f'pred_rels:{pred_rels}')
        # print(f'pred_rels shape:{pred_rels.shape}')
        # pred_scores = rel_scores[:,:-1].max(1)
        # rel_scores_1 = copy.deepcopy(rel_scores)
        # # rel_scores_1[pred_inds] = 0
        # # for idx, pred_idx in enumerate(pred_inds):
        # #     rel_scores_1[idx][pred_idx] = 0
        # rel_scores_1[np.arange(len(pred_inds)), pred_inds] = 0
        # pred_inds_1 = rel_scores_1[:,:-1].argmax(1)
        # pred_rels_1 = np.column_stack((pred_rel_inds, pred_inds_1))
        # pred_scores_1 = rel_scores_1[:,:-1].max(1)
        
        # rel_scores_2 = copy.deepcopy(rel_scores_1)
        # # print(f'pred_inds_1:{pred_inds_1}')
        # # print(f'rel_scores_2:{rel_scores_2}')
        # # print(f'pred_scores_1:{pred_scores_1}')
        # # print(rel_scores_2[pred_inds_1])
        # # rel_scores_2[pred_inds_1] = 0
        # # for idx, pred_idx in enumerate(pred_inds_1):
        # #     rel_scores_2[idx][pred_idx] = 0
        # rel_scores_2[np.arange(len(pred_inds_1)), pred_inds_1] = 0
        # # print(f'rel_scores_2 after:{rel_scores_2}')
        # pred_inds_2 = rel_scores_2[:,:-1].argmax(1)
        # pred_rels_2 = np.column_stack((pred_rel_inds, pred_inds_2))
        # pred_scores_2 = rel_scores_2[:,:-1].max(1)
        
        # # print(f'pred_rels_0 shape:{pred_rels_0.shape}')
        # # print(f'pred_scores_0 shape:{pred_scores_0.shape}')
        # pred_rels = np.vstack((pred_rels_0, pred_rels_1, pred_rels_2))
        # pred_scores = np.hstack((pred_scores_0, pred_scores_1, pred_scores_2))
        # print(f'pred_rels shape:{pred_rels.shape}')
        ...
        # print(f'gt_triplets:{gt_triplets.shape}')
        # print(f'pred_triplets:{pred_triplets}')
        # print(f'pred_triplets shape:{pred_triplets.shape}')
...
class SGNoGraphConstraintRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container, mode):
        obj_scores = local_container['obj_scores']
        pred_rel_inds = local_container['pred_rel_inds']
        rel_scores = local_container['rel_class_prob']
        pred_boxes = local_container['pred_boxes']
        pred_classes = local_container['pred_classes']
        gt_rels = local_container['gt_rels']
...
def _compute_pred_matches(gt_triplets, pred_triplets,
    ...
    # print(f'gt_triplets:{gt_triplets.shape}')
    # print(f'pred_triplets:{pred_triplets.shape}')
    if gt_triplets.shape[1] != pred_triplets.shape[1]:
        keeps = intersect_2d_topk(gt_triplets, pred_triplets)
    else:
        keeps = intersect_2d(gt_triplets, pred_triplets)
